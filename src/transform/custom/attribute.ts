...
export function transform_simpleAttrSyntax(segments: MacroInvocation['segments']) {
  ...
  function transform_segments<N extends boolean>(
          ...
          case NodeType.PunctuationToken:
            switch (seg.tk) {
              ...
              case TK['=']: {
                ...
                return rs.mockNode(
                  NodeType.ReassignmentExpression,
                  right.loc.cloneFrom(start(lhs)),
                  {
                    tk: TK['='],
                    kind: DelimKind['='],
                    left: lhs,
    ...
    function eatPathSegment(left: undefined | Identifier | ExpressionPath) {
      ...
      const res = rs.mockNode(
        NodeType.ExpressionPath,
        segment.loc.cloneFrom(start(left ?? seq[i])),
        { namespace: left, segment }
      );
      i += 2;
      return res;
    }
