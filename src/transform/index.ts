...
export function is_CallExpression_or_CallLikeMacroInvocation(
  node: any
): node is CallExpression | CallLikeMacroInvocation {
...
const transform: { [K in NodeType]?: (node: NTMap[K]) => void } = {
  ...
  [NodeType.CallExpression](node) {
    if (hasMethod(node)) {
      node.callee = rs.mockNode(
        NodeType.MemberExpression,
        node.method.loc.cloneFrom(start(node.callee)),
        {
    ...
    if (
      ...
      (false ||
        ...
        (!isReadingSnippet() &&
          ...
          !(is_ReassignmentNode(parent) && parent.left === node)))
    ) {
  ...
  function blockify(node: ExpressionNode) {
    const block = rs.mockNode(NodeType.BlockExpression, node.loc.clone(), {
      ...
      body: rs.createLocArray(DelimKind['{}'], node.loc.clone(), [
        rs.mockNode(NodeType.ExpressionStatement, node.loc.clone(), {
          semi: false,
          expression: node
        })
      ])
...
function flatten_typeBounds(topNode: Node) {
  if (hasTypeBounds(topNode)) {
    ...
    const flatten = (bound: TypeTraitBound) =>
      Array_replace(
        topNode.typeBounds,
        bound,
        ...(bound.typeExpression as unknown as TypeDynBounds).typeBounds
      );
    ...
    if (nestedBounds.every(isBareBoundWithNestedBoundsNoPrefix)) {
      ...
      if (is_TypeDynBounds(topNode)) {
        ...
        (first.typeExpression as unknown as TypeDynBounds).typeBounds.push(
          ...topNode.typeBounds.slice(1)
        );
  ...
  function isBoundWithNestedBounds(
    bound: TypeBound
  ): bound is TypeTraitBound & { typeExpression: TypeBoundsStandaloneNode } {
    return is_TypeTraitBound(bound) && is_TypeBoundsStandaloneNode(bound.typeExpression);
  }
  function isBareBoundWithNestedBounds(
    bound: TypeBound
  ): bound is TypeTraitBound & { typeExpression: TypeBoundsStandaloneNode } {
    return isBoundWithNestedBounds(bound) && is_BareTypeTraitBound(bound);
  }
  function isBareBoundWithNestedBoundsNoPrefix(
    bound: TypeBound
  ): bound is TypeTraitBound & { typeExpression: TypeDynBounds } {
  ...
  function getTransformedNodeChildren(node: Node) {
    ...
    __DEV__: {
      ...
      if (
        ...
        exit(
          `${node.type} was transformed but did not patch its childNodes list`,
          ctx,
          node.loc.url(),
          node
        );
      ...
      for (const child of children)
        if (!is_Node(child))
          exit(`${node.type}'s childNodes includes unexpected entries`, { node, child });
    }
    return children;
  }
}
