...
export function needsOuterSoftbreakParens(node: Node) {
  ...
  if (
    ...
    (false ||
      ...
      (is_ExpressionWithBodyOrCases_or_BlockLikeMacroInvocation(parent) &&
        !is_ElseBlock(node, parent)))
  ) {
    return true;
  }
  ...
  if (hasComment(node)) {
    ...
    if (hasComment(node, CF.Line)) {
      if (
        is_ReturnExpression(parent) ||
        (is_YieldExpression(parent) && parent.expression === node)
      ) {
        return true;
      }
    }
...
export function needsInnerParens(node: Node) {
  ...
  if (
    is_LetScrutinee(parent) &&
    is_LogicalExpression(node) &&
    parent.expression === (node as any)
  ) {
    return true;
  }
  ...
  if (is_ExpressionWithBodyOrCases_or_BlockLikeMacroInvocation(node)) {
    ...
    if (
      is_LetScrutinee(parent) &&
      parent.expression === node &&
      is_ExpressionWithBodyOrCases(getGrandParentNode())
    ) {
      return true;
    }
    return (
  ...
  if (
    is_LogicalExpression(node) ||
    is_OperationExpression(node) ||
    is_ComparisonExpression(node) ||
  ...
  if (is_TypeFunctionNode(node)) {
    ...
    if (
      ...
      is_TypeBoundsStandaloneNode(gp) &&
      last_of(gp.typeBounds) !== parent
    ) {
      return true;
    }
  }
...
function precedenceNeedsParens(
  node: LeftRightExpression | ClosureFunctionExpression | ExpressionAsTypeCast,
  parent: Node
) {
  ...
  if (is_LogicalExpression(parent))
    return is_LogicalExpression(node)
      ? parent.nodeType !== node.nodeType
      : evalPrecedence(node, parent);
  ...
  function evalPrecedence(
    ...
    if (
      is_BitwiseOperator(parent.tk) ||
      (is_BitwiseOperator(child.tk) && is_EqualityOperator(parent.tk))
    ) {
      return true;
    }
...
export function shouldFlatten(
  parent: ExpressionNode | ConditionExpression,
  node: ExpressionNode | ConditionExpression
) {
  if (
    getPrecedence(node, is_insideScrutinee(node)) !==
    getPrecedence(parent, is_insideScrutinee(parent))
  )
    return false;
  ...
  if (is_OperationExpression(parent) && is_OperationExpression(node)) {
    if (
      ...
      (node.tk !== parent.tk &&
        is_multiplicativeOperator(node.tk) &&
        is_multiplicativeOperator(parent.tk)) ||
...
export function needsSemi(
  parent: NodeWithBody,
  stmt: ExpressionStatement,
  disregardExprType = false
) {
  ...
  function shouldPreserveSemi() {
    return (
      stmt === last_of(parent.body!) &&
      (is_ImplicitReturnAbleNode(parent) || is_BlockLikeMacroInvocation(parent))
    );
  }
...
function canInlineInlineable(node: ExpressionWithBody) {
  ...
  if (
    ...
    (!is_ImplicitReturnAbleNode(node) ||
      pathCallAtParent(parent, (parent) => stmtNeedsSemi(parent, true)))
  ) {
    return false;
  }
