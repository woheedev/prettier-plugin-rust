...
export const isSimpleType = IsSimpleFunction<FunctionSpread | TypeNode | MissingNode>(
  (node): boolean => {
    switch (node.nodeType) {
      case NodeType.MissingNode:
      ...
      case NodeType.ExpressionTypeSelector:
        return (
          isSimpleType(node.typeTarget) &&
          (!node.typeExpression || isSimpleType(node.typeExpression))
        );
          ...
          (node.parameters.length === 0 ||
            (node.parameters.length === 1 &&
              (is_FunctionSpread(param) ||
                (!is_TypeFunctionNode(param.typeAnnotation) &&
                  isSimpleType(param.typeAnnotation))))) &&
          (!node.returnType || isSimpleType(node.returnType))
        );
      }
      case NodeType.TypeFunction:
        return (
          isSimpleType(node.callee) &&
          node.parameters.every(isSimpleType) &&
          (!node.returnType || isSimpleType(node.returnType))
        );
...
export const hasComplexTypeBounds = HasComplexFunction<Extract<Node, TypeBoundsConstaint>>(
  (node) => {
    return (
      !!node.typeBounds && node.typeBounds.length > 1 && !node.typeBounds.every(isSimpleTypeBound)
    );
  }
);
...
export const isSimpleTypeBound = (node: TypeBound): boolean => {
  switch (node.nodeType) {
    ...
    case NodeType.TypeTraitBound:
      return (
        is_BareTypeTraitBound(node) && isSimpleTypeNamespaceTargetNoSelector(node.typeExpression)
      );
    default:
      __DEV__: exit.never(node);
      return false;
  ...
  function isSimpleTypeNamespaceTargetNoSelector(node: TypeNamespaceTargetNoSelector): boolean {
    switch (node.nodeType) {
      ...
      case NodeType.TypePath:
        return (
          undefined === node.namespace || isSimpleTypeNamespaceTargetNoSelector(node.namespace)
        );
      case NodeType.TypeCall:
        return false;
      case NodeType.TypeFunction:
        return (
          isSimpleTypeNamespaceTargetNoSelector(node.callee) &&
          node.parameters.length === 0 &&
          !node.returnType
        );
      default:
...
export const hasComplexTypeArguments = HasComplexFunction<Extract<Node, MaybeGenericArgsTarget>>(
  (node) =>
...
export const hasComplexLtParameters = HasComplexFunction<Extract<Node, ForLtParametersBody>>(
  (node) => {
...
export const isShortGenericParameterDeclaration = IsSimpleFunction<GenericParameterDeclaration>(
  (node) => {
    switch (node.nodeType) {
...
export const hasComplexGenerics = HasComplexFunction<Node>((node) => {
  return (
    has_key_defined(node, 'generics') &&
    node.generics.length > 0 &&
    !node.generics.every(isShortGenericParameterDeclaration)
  );
});
