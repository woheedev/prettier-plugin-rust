...
export function setPrettierIgnoreTarget(node: Node, comment: AnyComment) {
  __DEV__: (Narrow<Node & { prettierIgnore?: true }>(node),
    assert(isPrettierIgnoreComment(comment) || isPrettierIgnoreAttribute(comment)));
...
function printComment(comment: AnyComment) {
  ...
  __DEV__: assert(
    handled(comment),
    `Assertion failed: Comment was not printed at ${comment?.loc?.url() || 'unknown'}`,
    comment
  );
    ...
    return hasBreaklineBefore(comment)
      ...
      : is_BlockCommentNode(comment)
        ...
        : lineSuffix([
            ' ',
            printed,
            hasTrailingLineComment === (hasTrailingLineComment = true) ? hardline : breakParent
          ]);
  }
}
...
export function withComments<D extends Doc>(
  node: Node,
  printed: D,
  ignored?: Set<AnyComment>
): D | Doc[] {
...
export function getComments(
  node: Node,
  ...args: Parameters<typeof getCommentTestFunction>
): AnyComment[] {
  __DEV__: Narrow<Node & { comments?: AnyComment[] }>(node);
...
function getCommentTestFunction(flags: CF, fn?: (comment: AnyComment) => boolean) {
  return function (comment: AnyComment, index: number, comments: AnyComment[]) {
    ...
    return !(
      ...
      (flags & CF.PrettierIgnore &&
        !(isPrettierIgnoreComment(comment) || isPrettierIgnoreAttribute(comment))) ||
      (fn && !fn(comment))
    );
  };
}
...
export function hasComment(node: Node, flags: number = 0, fn?: (comment: AnyComment) => boolean) {
  if ('comments' in node && node.comments!.length > 0) {
    return flags || fn
      ? (node.comments as AnyComment[]).some(getCommentTestFunction(flags, fn))
      : true;
  }
  return false;
}
...
function handled(comment: AnyComment | undefined): boolean {
  return !!(comment && 'printed' in comment);
}
function handleCommon(ctx: CommentContext): boolean {
  {
    const { comment, precedingNode, enclosingNode, followingNode } = ctx;
    if (!enclosingNode) {
      ctx.enclosingNode = comment?.loc?.src?.program;
      ...
      if (body) {
        ...
        if (comment?.loc?.isBefore(body)) {
          if (followingNode && followingNode.loc && body.loc.contains(followingNode)) {
            ctx.followingNode = undefined;
          }
          ...
          if (precedingNode && precedingNode.loc && body.loc.contains(precedingNode)) {
            ctx.precedingNode = undefined;
          }
          if (!ctx.precedingNode && !ctx.followingNode) {
        ...
        } else if (comment?.loc && body.loc.contains(comment)) {
          if (precedingNode && precedingNode.loc && !body.loc.contains(precedingNode)) {
            ctx.precedingNode = undefined;
          }
          if (followingNode && followingNode.loc && !body.loc.contains(followingNode)) {
  ...
  if (isStartOfLine(comment)) {
    if (followingNode && followingNode.loc) {
      addLeadingComment(followingNode, comment);
    } else if (precedingNode && precedingNode.loc) {
      addTrailingComment(precedingNode, comment);
    } else {
      exit.never(ctx);
    }
  } else if (isEndOfLine(comment)) {
    if (precedingNode && precedingNode.loc) {
      addTrailingComment(precedingNode, comment);
    } else if (followingNode && followingNode.loc) {
      addLeadingComment(followingNode, comment);
    } else {
      exit.never(ctx);
    }
  } else {
    if (precedingNode && precedingNode.loc && followingNode && followingNode.loc) {
      return false;
    } else if (precedingNode && precedingNode.loc) {
      addTrailingComment(precedingNode, comment);
    } else if (followingNode && followingNode.loc) {
...
export function handleEndOfLineComment(ctx: CommentContext) {
  ...
  if (
    ...
    precedingNode ===
      (enclosingNode.typeArguments ? last_of(enclosingNode.typeArguments) : enclosingNode.callee)
  ) {
...
function handleStructLiteralComments({ enclosingNode, followingNode, comment }: CommentContext) {
  if (
    ...
    enclosingNode &&
    enclosingNode.loc &&
    is_StructLiteralPropertySpread(enclosingNode) &&
    followingNode === enclosingNode.expression
  ) {
    addLeadingComment(enclosingNode, comment);
  }
}
...
function handleMixedInOuterAttributeComments({
  precedingNode,
  enclosingNode,
  followingNode,
  comment
  ...
  if (
    ...
    enclosingNode &&
    enclosingNode.loc &&
    hasOuterAttributes(enclosingNode) &&
    end(comment) <= ownStart(enclosingNode)
  ) {
    ...
    if (isEndOfLine(comment)) {
      ...
      if (followingNode && followingNode.loc && end(followingNode) <= ownStart(enclosingNode)) {
        addLeadingComment(followingNode, comment);
      } else if (precedingNode && precedingNode.loc && enclosingNode.loc.contains(precedingNode)) {
        addTrailingComment(precedingNode, comment);
      } else if (enclosingNode && enclosingNode.loc) {
        addLeadingComment(enclosingNode, comment);
      }
    }
  }
}
function handleAttributeComments({
  precedingNode,
  enclosingNode,
  followingNode,
  comment,
  ...
  if (comment && is_AttributeOrDocComment(comment)) {
    if (
      comment.inner &&
      enclosingNode &&
...
function handleBadComments({
  ...
  if (!enclosingNode) {
    ...
    if (enclosingNode && enclosingNode.loc && enclosingNode !== ast) {
      addLeadingComment(enclosingNode, comment);
    } else {
      addDanglingComment(ast, comment, DCM['body']);
    }
...
function is_ABI_Comment({ precedingNode, enclosingNode, comment }: CommentContext) {
  ...
  return (
    ...
    ((precedingNode && is_ExternSpecifier(precedingNode)) ||
      (enclosingNode && is_ExternSpecifier(enclosingNode)))
  );
}
function handleFlowControlComments({
  precedingNode,
  enclosingNode,
  followingNode,
  comment
}: CommentContext) {
  if (comment && enclosingNode && enclosingNode.loc && is_FlowControlExpression(enclosingNode)) {
...
function handleFunctionComments(ctx: CommentContext) {
  ...
  if (comment && comment.loc && enclosingNode && is_FunctionNode(enclosingNode)) {
    if (
      is_FunctionDeclaration(enclosingNode) &&
      ((!is_ABI_Comment(ctx) && comment.loc.isBefore(enclosingNode.generics || enclosingNode.id)) ||
        (enclosingNode.generics &&
          comment.loc.isBetween(enclosingNode.generics, enclosingNode.parameters)))
    ) {
      addLeadingComment(enclosingNode, comment);
    } else if (
      ...
      (!is_FunctionDeclaration(enclosingNode) ||
        !enclosingNode.whereBounds ||
        comment.loc.isAfter(enclosingNode.whereBounds!)) &&
      (!enclosingNode.returnType || comment.loc.isAfter(enclosingNode.returnType)) &&
      followingNode ===
        (is_FunctionDeclaration(enclosingNode) ? enclosingNode.body?.[0] : enclosingNode.expression)
    ) {
      addLeadingComment(followingNode, comment);
    }
  }
...
function handleMacroRuleComments(ctx: CommentContext) {
  const { precedingNode, enclosingNode, followingNode, comment } = ctx;
  if (comment && comment.loc && enclosingNode && enclosingNode.loc && is_MacroRule(enclosingNode)) {
    if (enclosingNode.transform.loc.contains(comment)) {
      __DEV__: assert(enclosingNode.transform.length > 0);
      if (
        !precedingNode ||
        !precedingNode.loc ||
        !enclosingNode.transform.loc.contains(precedingNode)
      ) {
        __DEV__: assert(
          !!followingNode &&
            followingNode.loc &&
            enclosingNode.transform.loc.contains(followingNode)
        );
        addLeadingComment(followingNode, comment);
      }
      ...
      if (
        !followingNode ||
        !followingNode.loc ||
        !enclosingNode.match.loc.contains(followingNode)
      ) {
        __DEV__: assert(
          !!precedingNode && precedingNode.loc && enclosingNode.match.loc.contains(precedingNode)
        );
        addTrailingComment(precedingNode!, comment);
      }
    }
...
function handleStatementComments(ctx: CommentContext) {
  ...
  if (
    ...
    precedingNode &&
    precedingNode.loc &&
    (is_StatementNode(precedingNode) || precedingNode.loc.sliceText().endsWith(';'))
  ) {
    addTrailingComment(precedingNode, comment);
  }
}
...
function handleIfBlockExpressionComments(ctx: CommentContext) {
  ...
  if (
    ...
    enclosingNode &&
    enclosingNode.loc &&
    is_IfBlockExpression(enclosingNode)
  ) {
...
function handleMemberExpressionComments({ comment, precedingNode, enclosingNode }: CommentContext) {
  if (comment && enclosingNode && enclosingNode.loc && is_MemberAccessLike(enclosingNode)) {
    if (isStartOfLine(comment) || !precedingNode) addLeadingComment(enclosingNode, comment);
    else if (precedingNode && precedingNode.loc) addTrailingComment(precedingNode, comment);
    return true;
  }
...
function handleDanglingComments({ comment, enclosingNode }: CommentContext) {
  if (comment && comment.loc && enclosingNode) {
    for (var key in DCM) {
      if (key in enclosingNode) {
...
function isStartOfLine(comment: AnyComment) {
  return comment && comment.placement === 'ownLine';
}
function isEndOfLine(comment: AnyComment) {
  return comment && comment.placement === 'endOfLine';
}
...
export function print_comment(comment: CommentOrDocComment) {
  ...
  const doc = is_BlockCommentNode(comment)
    ? isIndentableBlockComment(comment.value)
      ? [
          (!handled(comment) || isTrailing(comment)) && !hasBreaklineBefore(comment)
            ? hardline
            : '',
