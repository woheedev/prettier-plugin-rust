...
export const printer: { [K in NodeType]: nPrint<Extract<NTMap[K], Node>> } = {
  ...
  [NodeType.MacroInvocation](print, node) {
    ...
    if (node.segments.length === 0) {
      return [
        ...
        hasCurlyBrackets ? ' ' : '',
        delim.left,
        printDanglingCommentsForInline(node),
        delim.right
      ];
    }
    if (isTransformed(node)) {
      if (is_CallLikeMacroInvocation(node)) {
        return [
          print('callee'),
          '!',
          printCallArguments(print as print<CallLikeMacroInvocation>, node)
        ];
      }
      if (is_BlockLikeMacroInvocation(node)) {
        return [
          ...
          '!',
          ' ',
          printBlockBody(print as print<BlockLikeMacroInvocation>, node)
        ];
      }
    }
  ...
  [NodeType.DestructuredImport](print, node) {
    if (node.specifiers.length === 0)
      return [print('source'), '::{', printDanglingCommentsForInline(node, DCM['specifiers']), '}'];
    let space = true;
  ...
  [NodeType.FunctionSelfParameterDeclaration](print, node) {
    return group([
      ...
      print.b('mut'),
      'self',
      printTypeAnnotation(print, node)
    ]);
  },
  ...
  [NodeType.StructDeclaration](print, node) {
    return [
      print('pub'),
      'struct',
      printGenerics_x_whereBounds(print, node, ''),
      printObject(print, node)
    ];
  },
  ...
  [NodeType.TupleStructDeclaration](print, node) {
    return [
      print('pub'),
      'struct',
      printGenerics_x_whereBounds(print, node, printArrayLike(print, node)),
      ';'
    ];
  },
  ...
  [NodeType.ImplDeclaration](print, node) {
    return [
      ...
      printGenerics_x_whereBounds(print, node, [
        print.b('const'),
        printImplTraitForType(print, node)
      ]),
      adjustClause(node, printBlockBody(print, node))
    ];
  },
  ...
  [NodeType.ParenthesizedExpression](print, node) {
    ...
    const shouldHug =
      ...
      (is_ArrayOrTupleLiteral(node.expression) || is_StructLiteral(node.expression));
  ...
  [NodeType.MatchExpressionCase](print, node) {
    return group([
      ...
      !hasComment(node.expression, 0, (comment) =>
        getOptions().danglingAttributes.includes(comment as any)
      )
        ? [' ', print('expression')]
  ...
  [NodeType.TypeTraitBound](print, node) {
    return [
      print.b('maybeConst', '~const '),
      print.b('optional', '?'),
      printLtParameters(print, node),
  ...
  [NodeType.TypeFnPointer](print, node) {
    return [
      ...
      print('extern'),
      'fn',
      printParametersAndReturnType(node)
    ];
  },
  ...
  [NodeType.TypeSizedArray](print, node) {
    ...
    if (isSimpleType(node))
      return ['[', print('typeExpression'), '; ', print('sizeExpression'), ']'];
  },
