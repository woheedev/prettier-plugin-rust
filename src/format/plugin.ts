...
export function is_printing_macro() {
  return getContext().path.stack.some(
    (node) => is_Node(node) && (is_MacroInvocation(node) || is_Attribute(node))
  );
}
...
export function assertPathAtNode(name: string, node: Node, ...ctx: any[]) {
  __DEV__: if (getNode() !== node)
    exit(
      `Attempted to call ${name}() in wrong prettier path context`,
      { asserted: node, actual: getNode() },
      ...ctx
    );
}
...
export function pathCall<T extends Node, K extends keyof NodeProps<T> & keyof T, R>(
  node: T,
  key: K,
  fn: (child: T[K]) => R
): R {
...
export function pathCallTopMostIfBlockExpression<R>(
  node: IfBlockExpression,
  fn: (node: IfBlockExpression) => R
): R {
  const parent = getParentNode(node)!;
  return is_ElseBlock(node, parent)
    ? pathCallAtParent(parent, (parent) => pathCallTopMostIfBlockExpression(parent, fn))
    : fn(node);
}
...
function print(property?: any, args?: any): Doc | Doc[] {
  ...
  return !!value
    ? Array.isArray(value)
      ? ctx.path.map(ctx.print, property)
      : ctx.print(property, args)
    : '';
}
...
namespace print {
  ...
  export function map(property: string, mapItem?: MapFn<any, any>): Doc[] {
    return !has(property)
      ? []
      : ctx.path.map(mapItem ? (p, i, a) => mapItem(a[i], i, a) : () => ctx.print(), property);
  }
  export function join(
    property: string,
    sep: SepFn<any, any> | Doc,
    trailingSep: TrailingSepFn<any, any> | Doc = ''
  ): Doc[] {
...
type MapFn<T extends Node = Node, K extends AK<T> = AK<T>> = <A extends AV<T, K>>(
  item: A[number],
  index: number,
  arr: A
) => Doc;
type TrailingSepFn<T extends Node = Node, K extends AK<T> = AK<T>> = <A extends AV<T, K>>(
  item: A[number]
) => Doc;
...
export interface print<T extends Node> {
  ...
  join<K extends AK<T>>(
    property: K,
    sep: SepFn<T, K> | Doc,
    trailingSep?: TrailingSepFn<T, K> | Doc
  ): Doc[];
  map_join<K extends AK<T>>(
    property: K,
    mapFn: MapFn<T, K>,
    sep: SepFn<T, K> | Doc,
    trailingSep?: TrailingSepFn<T, K> | Doc
...
function genericPrint() {
  return withCheckContext(() => {
    ...
    if (hasAttributes(node)) {
      const print_above =
        shouldPrintOuterAttributesAbove(node); /*  || node.attributes.length > 1 */
      printed = [
        ...print.join(
          'attributes',
          (attr) =>
          ...
          (attr) =>
            print_above && is_DocCommentAttribute(attr)
              ? maybeEmptyLine(attr)
              : print_above ||
                  is_LineCommentNode(attr) ||
                  (is_BlockCommentNode(attr) && hasBreaklineAfter(attr))
                ? hardline
                : ' '
        ),
        printed
      ...
      hasPrettierIgnore(node) ||
        ...
        : is_MacroRule(node)
          ...
          : is_UnionPattern(getParentNode() ?? ({ nodeType: 0 } as any))
            ? new Set(getComments(node, 0 as CF, (comment) => !isDangling(comment)))
            : undefined
    );
...
export function canAttachComment(n: Node) {
  return (
    !is_Comment(n) && !isNoopExpressionStatement(n) && !is_MissingNode(n) && !is_PunctuationToken(n)
  );
}
...
export const plugin: Plugin<Node> = {
  ...
  parsers: {
    'jinx-rust': {
      ...
      locStart: start,
      locEnd: end,
      parse(code: string, parsers: any, options: CustomOptions): Program {
        ctx = { options } as any;

        options.rsParsedFile = rs.parseFile((options.originalText = code), {
      ...
      const msg =
        color.magenta(
          (comment.marker ? `Dangling "${comment.marker}" ` : '') +
            (is_Attribute(comment)
              ? 'Attribute '
              : is_DocCommentAttribute(comment)
                ? 'DocCommentAttribute'
                : 'Comment') +
            ` ${index}/${comments.length}` +
            color.yellow(
              ` ${print_string(comment.loc.sliceText(0, len))}${comment.loc.len() > len ? ' ...' : ''}`
            )
        ) + color.grey(`\n    at ${comment.loc.url()}`);
