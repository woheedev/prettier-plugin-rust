...
export function isNoopExpressionStatement(node: Node) {
  return (
    is_ExpressionStatement(node) &&
    undefined === node.expression &&
    !hasAttributes(node) &&
    !hasComment(node)
  );
}
...
export function is_BinaryishExpression(
  node: Node
): node is OrExpression | AndExpression | OperationExpression | ComparisonExpression {
  switch (node.nodeType) {
    ...
    (node.items as Node[]).every(
      (element) =>
        (is_LiteralNumberLike(element) ||
          (is_MinusExpression(element) &&
            is_LiteralNumberLike(element.expression) &&
            !hasComment(element.expression))) &&
...
export function printBodyOrCases<T extends NodeWithBodyOrCases | BlockLikeMacroInvocation>(
  print: print<T>,
  node: T
) {
  ...
  if (is_MatchExpression(node)) {
    ...
    pathCallEach(node as Extract<Node, MatchExpression>, 'cases', (mCase) => {
      p.push({
        node: mCase,
        doc:
          is_MatchExpressionCase(mCase) && !is_ExpressionWithBodyOrCases(mCase.expression)
            ? [print(), ',']
            : print()
      });
    });
...
export function printMacroRules<T extends MacroDeclaration | MacroRulesDeclaration>(
  print: print<T>,
  node: T
) {
  return !Array.isArray(node.rules)
    ...
    : node.rules.length > 0
      ? [
          ' {',
          indent([hardline, ...print.join('rules', (rule) => maybeEmptyLine(rule))]),
          hardline,
          '}'
        ]
...
function can_unary(node: MacroMatchSegment) {
  return (
    (!is_PunctuationToken(node) || is_unary_token(node)) &&
    (!is_MacroGroup(node) || is_optional_unary(node))
  );
}
function is_optional_token(
  item: MacroMatchSegment | undefined
): item is MacroGroup<MacroMatchSegment> & { segments: [PunctuationToken] } {
  return (
    !!item &&
    is_MacroGroup(item) &&
    item.kind === '?' &&
    item.segments.length === 1 &&
    is_PunctuationToken(item.segments[0])
  );
}
...
export function printRuleMatch<T extends MacroRuleDeclaration | MacroInlineRuleDeclaration>(
  print: print<T>,
  rule: T
) {
  // "", ".", "&&", "||", "=", "+", "-", "*", "/", "%", "&", "|", "^", "<<", ">>", "==", "!=", ">", ">=", "<", "<=", "+=", "-=", "*=", "/=",
  ...
  type ArrProps<T extends {}> = {
    [K in keyof T]: NonNullable<T[K]> extends readonly any[] ? T[K] & unknown[] : never;
  };
  function print_map<T extends Node, K extends keyof ArrProps<T> & keyof typeof DCM>(
    node: T,
    property: K
  ) {
    ...
    const dline =
      arr.dk === DelimKind['{}']
        ? line
        : shouldHug
          ? '' /*  : is_MacroGroup(node) && node.kind !== "?" ? hardline */
          : softline;
    ...
    function should_hug(arr: LocArray<MacroMatchSegment>) {
      ...
      return arr.every(
        (item) =>
          !is_MacroGroup(item) &&
          (is_PunctuationToken(item) || has_nonToken !== (has_nonToken = true))
      );
    }
      ...
      function printComments(doc: Doc) {
        ...
        return comment && index !== 0
          ? isPreviousLineEmpty(comment) &&
            typeof join_item(arr[index - 1], item, index === 1 ? undefined : arr[index - 2]) ===
              'string'
    ...
    function join_item(
      item: MacroMatchSegment,
      next: MacroMatchSegment,
      prev: MacroMatchSegment | undefined
    ) {
      ...
      {
        const sep_tk =
          is_MacroGroup(item) && item.sep && is_PunctuationToken(item.sep) ? item.sep.tk : TK.None;
        switch (sep_tk) {
          case TK['::']:
          case TK['.']:
            return ''; // $(...)::* | $(...).*
          case TK[',']:
          case TK[';']:
            return sep_tk === maybe_tk(next)
              ? ifBreak(line, ' ', { groupId: getMacroGroupId(item) })
              : line;
        }
      }
      ...
      if (is_DelimGroup(item) || is_MacroGroup(item)) {
        ...
        if (is_MacroGroup(item) && item.segments.length === 2) {
          ...
          if (
            is_PunctuationToken(left) &&
            is_DelimGroup(right) &&
            left.tk === TK['#'] &&
            right.segments.dk === DelimKind['[]']
          ) {
            return hardline;
          }
        }
...
function hasLeadingOwnLineComment(node: Node): boolean {
  ...
  return hasComment(
    ...
    CF.Leading,
    (comment) =>
      hasNewline(end(comment)) && !getContext().options.danglingAttributes.includes(comment as any)
  );
}
  ...
  if (is_ReassignmentExpression(node)) {
    ...
    return (
      ...
      leftNode.properties.some(
        (property) => is_StructLiteralProperty(property) || is_StructLiteralPropertySpread(property)
      )
    );
  }
...
function isArrowFunctionVariableDeclarator(node: Node) {
  return (
    is_VariableDeclarationNode(node) &&
    node.expression &&
...
function isObjectPropertyWithShortKey(node: Node, keyDoc: Doc) {
  ...
  return (
    ...
    keyDoc.length < getContext().options.tabWidth + MIN_OVERLAP_FOR_BREAK
  );
}
...
export function printCallExpression(print: print<CallExpression>, node: CallExpression) {
  if (
    shouldPrint_CallExpression_chain(node) &&
    !pathCall(node, 'callee', (node) => needsParens(node))
  ) {
...
export function printTypeAnnotation<T extends Extract<Node, MaybeTypeAnnotationTarget<any>>>(
  print: print<T>,
  node: T
) {
  return node.typeAnnotation && !is_MissingNode(node.typeAnnotation)
    ? [': ', print('typeAnnotation')]
    : '';
}
export function printAnnotatedPattern<
  T extends Extract<Node, MaybeTypeAnnotationTarget<any> & PatternBody>
>(print: print<T>, node: T) {
...
function isLoneShortArgument(node: Node) {
  ...
  if (
    (is_Identifier(node) && is_short(node.name)) ||
    (is_LiteralNumberLike(node) && !hasComment(node))
  ) {
    return true;
  }
...
function shouldInlineMemberExpression(node: MemberExpression, objectDoc: Doc) {
  ...
  const shouldInline =
    (nmparent &&
      (is_ExpressionPath(nmparent) ||
        (is_ReassignmentNode(nmparent) && !is_Identifier(nmparent.left)))) ||
    !node.computed ||
    (is_Identifier(node.expression) &&
      is_Identifier(node.property) &&
      !is_MemberExpression(parent)) ||
    (is_AssignmentOrVariableDeclarator(parent) &&
      ((is_CallExpression_or_CallLikeMacroInvocation(node.expression) &&
...
export function printAssignment(leftDoc: Doc, operator: string, rightPropertyName: string) {
  ...
  const rightDoc = getPrintFn(assignmentNode)(rightPropertyName as any, {
  ...
  function chooseLayout() {
    ...
    const shouldUseChainFormatting = getContext().path.match(
      ...
      (node: Node) =>
        !isTail || (!is_ExpressionStatement(node) && !is_VariableDeclarationNode(node))
    );

    if (shouldUseChainFormatting) {
      return !isTail
        ? Layout['chain']
        : is_ClosureFunctionExpression(rightNode) &&
            is_ClosureFunctionExpression(rightNode.expression)
    ...
    if (
      pathCall(assignmentNode, rightPropertyName as never, (rightNode) =>
        shouldBreakAfterOperator(rightNode, hasShortKey)
      )
    ) {
      return Layout['break-after-operator'];
    }
  ...
  function shouldBreakAfterOperator(rightNode: Node, hasShortKey: boolean) {
    if (
      is_MemberExpression(rightNode) &&
      shouldInlineMemberExpression(rightNode, getPrintFn(rightNode)('expression'))
    ) {
      return false;
    }
      ...
      return (function unwrap(node: Node): boolean {
        ...
        if (is_CallExpression_or_CallLikeMacroInvocation(node)) {
          ...
          const isPoorlyBreakableCall =
            args.length === 0 || (args.length === 1 && isLoneShortArgument(args[0]));
...
function is_MemberExpression_with_RangeOrLiteral_Property(node: Node | undefined) {
  return (
    !!node &&
    is_MemberExpression(node) &&
    (node.computed ? is_Literal_or_SimpleRangeLiteral(node.property) : is_Literal(node.property))
  );
}
...
function shouldPrint_CallExpression_chain(node: CallExpression) {
  return (
    is_MemberAccessLike(node.callee) || is_CallExpression_or_CallLikeMacroInvocation(node.callee)
  );
}
  ...
  return (
    ...
    (callExpressions.length > 2 &&
      callExpressions.some((expr) =>
        expr.arguments.some((arg) => !isSimpleCallArgument(arg, 0))
      )) ||
...
function printMemberChain(print: print<CallExpression>, node: CallExpression) {
  ...
  const shouldHaveEmptyLineBeforeIndent =
    !is_CallExpression_or_CallLikeMacroInvocation(lastNodeBeforeIndent) &&
  ...
  const callExpressions = printedNodes
    .map(({ node }) => node)
    .filter(is_CallExpression_or_CallLikeMacroInvocation);
  ...
  const result: Doc = useExpanded
    ? group(expanded)
    : [
        shouldHaveEmptyLineBeforeIndent || willBreak(oneLine) ? breakParent : '',
  ...
  function shouldNotWrap(groups: ChainItem[][]) {
    const hasComputed =
      groups[1].length > 0 && is_MemberExpression(groups[1][0].node) && groups[1][0].node.computed;

    if (groups[0].length === 1) {
      const firstNode = groups[0][0].node;
      return (
        is_Identifier(firstNode) &&
    ...
    const lastNodeLeft = is_ExpressionPath(lastNode)
      ...
      : is_MemberExpression(lastNode)
        ...
        : undefined;
    return (
      lastNodeLeft && is_Identifier(lastNodeLeft) && (isFactory(lastNodeLeft.name) || hasComputed)
    );
  }
...
function isSimpleCallArgument(node: Node, depth: number): boolean {
  ...
  if (is_MemberExpression(node)) {
    return (
      isSimpleCallArgument(node.expression, depth) && isSimpleCallArgument(node.property, depth)
    );
  }
...
export function printTypeArguments<T extends Extract<Node, MaybeGenericArgsTarget>>(
  print: print<T>,
  node: T
) {
...
export function printLtParameters<T extends Extract<Node, ForLtParametersBody>>(
  print: print<T>,
  node: T
) {
...
function getPrintedTypeBounds<T extends Extract<Node, TypeBoundsConstaint>>(
  print: print<T>,
  node: T
) {
...
export function printTypeBounds<T extends Extract<Node, TypeBoundsConstaint>>(
  operator: 'dyn' | 'impl' | ':',
  print: print<T>,
  node: T
) {
...
export function printLtBounds<T extends Extract<Node, MaybeHasLtBounds>>(
  left: Doc,
  print: print<T>,
  node: T
) {
    ...
    !node.ltBounds
      ...
      : node.ltBounds.length === 0
        ...
        : [
            left,
            ' ',
            print.map('ltBounds', (typeBound, i) =>
              i === 0 ? print() : indent([line, '+ ', print()])
            )
          ]
  );
}
...
export function printDeclarationTypeBounds<T extends Extract<DeclarationNode, TypeBoundsConstaint>>(
  ...
  return hasTypeBounds(node)
    ? adjustDeclarationClause(node, operator, getPrintedTypeBounds(print, node))
    : '';
}
...
function adjustDeclarationClause(
...
export function printDanglingCommentsForInline(node: Node, marker?: DCM) {
  const hasOnlyBlockComments =
    !hasComment(node, CF.Line | CF.Dangling, (comment) => !marker || comment.marker === marker) ||
    is_Program(node);
...
function isFormatLikeCall(node: CallExpression | CallLikeMacroInvocation) {
  if (is_Identifier(node.callee) && !node.typeArguments) {
    ...
    if (
      is_Literal(first) &&
      is_LiteralStringLike(first) &&
      first.value.includes('{}') &&
      rest.every(is_Identifier)
    ) {
      return true;
    }
  }
...
export function printCallArguments<T extends CallExpression | CallLikeMacroInvocation>(
  print: print<T>,
  node: T
) {
  const args = node.arguments;
  ...
  if (args.length === 0)
    return [LEFT, printDanglingCommentsForInline(node, DCM['arguments']), RIGHT];
      ...
      conditionalGroup([
        ...
        shouldGroupFirst
          ...
          : [
              LEFT,
              ...printedArguments.slice(0, -1),
              group(printedExpanded[lastArgIndex], { shouldBreak: true }),
              RIGHT
            ],
        allArgsBrokenOut()
      ])
  ...
  function allArgsBrokenOut() {
    return group([LEFT, indent([line, ...printedArguments]), trailingComma, line, RIGHT], {
      shouldBreak: true
    });
  }
}
...
function shouldGroupFunctionParameters(functionNode: FunctionDeclaration, returnTypeDoc: Doc) {
  ...
  return (
    ...
    (willBreak(returnTypeDoc) ||
      willBreak(printWhereBounds(getPrintFn(functionNode), functionNode)))
  );
}

export function printBlockBody<T extends NodeWithBodyOrCases | BlockLikeMacroInvocation>(
  print: print<T>,
  node: T
): Doc {
...
export function printMaybeBlockBody<
  T extends (NodeWithBodyOrCases | BlockLikeMacroInvocation) & { body: undefined | {} }
>(print: print<T>, node: T): Doc {
...
export function printArrowFunction<T extends ClosureFunctionExpression>(print: print<T>, node: T) {
  ...
  (function rec(node: ClosureFunctionExpression) {
    ...
    chainShouldBreak ||=
      !!node.returnType || !node.parameters.every((param) => isSimplePattern(param.pattern));
...
function printArrowChain(
  ...
  shouldBreak: boolean,
  bodyDoc: Doc,
  tailNode: ClosureFunctionExpression
) {
  const { args } = getContext();
  const parent = getParentNode()!;
  const isCallee =
    is_CallExpression_or_CallLikeMacroInvocation(parent) && parent.callee === getNode();
  const isAssignmentRhs = !!(args && args.assignmentLayout);
  const shouldPutBodyOnSeparateLine =
    !is_ExpressionWithBodyOrCases(tailNode.expression) && !is_StructLiteral(tailNode.expression);
  const shouldBreakBeforeChain =
    (isCallee && shouldPutBodyOnSeparateLine) ||
    (args && args.assignmentLayout === Layout['chain-tail-arrow-chain']);
...
function printArrowFunctionSignature<T extends ClosureFunctionExpression>(
  print: print<T>,
  node: T
) {
  const { args } = getContext();
...
export function printGenerics_x_whereBounds<T extends DeclarationNode>(
  print: print<T>,
  node: T,
  xDoc: Doc
) {
...
export function adjustClause(
  node:
    | DeclarationNode
    | ((NodeWithBodyOrCases | BlockLikeMacroInvocation) & { body: undefined | {} }),
  doc: Doc
) {
  return [
    'whereBounds' in node &&
    (!!node.whereBounds || (hasTypeBounds(node) && node.typeBounds.length > 1)) &&
    willBreak(doc)
...
export function printFlowControlExpression<
  T extends ReturnExpression | BreakExpression | YieldExpression
>(print: print<T>, node: T) {
...
export function flowControlExpressionNeedsOuterParens(
  flow: ReturnExpression | BreakExpression | YieldExpression
) {
  return (
    flow.expression &&
    (function hasLeadingComment(node: Node) {
...
export function printReturnType<T extends Extract<Node, MaybeReturnTypeConstraint>>(
  print: print<T>,
  node: T
) {
  return node.returnType
...
function printFunctionParameters<T extends FunctionNode | TypeFunctionNode>(
  ...
  const generics =
    printTypeParams && is_FunctionDeclaration(node) ? printGenerics(print as any, node) : '';
  ...
  if (hasSelfParameter(node)) {
    printed.unshift(
      getContext().path.call(
        () => [print(), printed.length ? sepFn(node.parameters.self) : ''],
        'parameters',
        'self'
      )
    );
...
function shouldGroupLastArg(args: LocArray<ExpressionNode>): boolean {
  ...
  return (
    ...
    (args.length !== 2 ||
      !is_ClosureFunctionExpression(preLastArg) ||
      !is_ArrayOrTupleLiteral(lastArg)) &&
...
export function printBinaryishExpression<
  T extends AndExpression | OrExpression | ComparisonExpression | OperationExpression
>(print: print<T>, node: T) {
  const parent = getParentNode()!;
  const grandParent = getGrandParentNode();
  const isInsideParenthesis =
    ('condition' in parent && parent.condition === node) || is_MatchExpression(parent);
  ...
  const shouldIndentIfInlining =
    is_ReassignmentNode(parent) ||
    is_VariableDeclarationNode(parent) ||
    is_StructLiteral(parent) ||
    is_StructLiteral(grandParent);

  const samePrecedenceSubExpression =
    is_BinaryishExpression(node.left) && shouldFlatten(node, node.left);
  ...
  const firstGroupIndex = parts.findIndex(
    (part) => typeof part !== 'string' && !Array.isArray(part) && part.type === 'group'
  );
  ...
  function printBinaryishExpressions(isNested: boolean, isInsideParenthesis: boolean): Doc[] {
    ...
    if (shouldFlatten(node, node.left)) {
      parts.push(
        ...pathCall(node, 'left', () => printBinaryishExpressions(true, isInsideParenthesis))
      );
    } else {
...
export function printIfBlockCondition<T extends IfBlockExpression | MatchExpressionCase>(
  print: print<T>,
  node: T
) {
...
export function printCondition<
  T extends IfBlockExpression | MatchExpressionCase | WhileBlockExpression
>(print: print<T>, node: T) {
...
function unwrapParenthesized(doc: Doc) {
  __DEV__: {
    ...
    assert(
      Array.isArray(doc) && check(doc.length === 1 ? doc[0] : doc),
      "Expected ['(', Doc, ')']",
      doc
    );
  }
...
export function withParensIdentOnBreak(node: Node, printed: Doc) {
  ...
  const grouped = group([
    indent([softline, parens ? unwrapParenthesized(printed) : printed]),
    softline
  ]);
  return conditionalGroup([printed, parens ? ['(', grouped, ')'] : grouped], {
...
function isSimplePattern(node: PatternNode | undefined) {
  ...
  switch (node.nodeType) {
    ...
    case NodeType.ExpressionTypeSelector:
      return (
        is_Identifier(node.typeTarget) &&
        (!node.typeExpression || is_Identifier(node.typeExpression))
      );
    case NodeType.ExpressionPath:
      return !node.namespace || isSimplePattern(node.namespace);
    case NodeType.RangePattern:
      return (
        (!node.lower || isSimplePattern(node.lower)) && (!node.upper || isSimplePattern(node.upper))
      );
...
export function printUnionPattern<T extends UnionPattern>(print: print<T>, node: T) {
  ...
  const parent = getParentNode();
  const prebreak =
    parent && (is_VariableDeclarationNode(parent) || is_LetScrutinee(parent)) && !needsParens(node);
  return group([
    prebreak ? softline : '',
...
export function printArrayLike<T extends ArrayLikeNode>(print: print<T>, node: T) {
  ...
  const shouldBreak =
    ...
    (node.items as (ExpressionNode | PatternNode)[]).every((item, i) => {
      ...
      return (
        ((hasProperties(item) && item.properties.length > 1) ||
          (hasItems(item) && item.items.length > 1)) &&
          ...
          (item, next) =>
            isNextLineEmpty(item)
              ...
              : hasComment(
                    next,
                    CF.Leading,
                    (comment) => is_LineCommentNode(comment) || comment.placement === 'ownLine'
                  )
                ? [',', hardline]
                : [',', line],
          trailingComma
  ...
  return group(
    [
      ...
      printDanglingComments(node, true, DCM['items']),
      softline,
      delims.right
    ],
    {
...
export function printObject<T extends ObjectNode>(print: print<T>, node: T): Doc {
  ...
  if (!hasProperties(node)) {
    return [
      ' {',
      printDanglingCommentsForInline(node, DCM['properties']) || emptyContent(node),
      '}'
    ];
  }
  ...
  if (
    ...
    (is_StructPattern(node) &&
      (is_VariableDeclarationNode(parent) ||
        is_MatchExpressionCase(parent) ||
        is_FunctionParameterDeclaration(parent)) &&
      parent.pattern === node)
  ) {
    return content;
  }
