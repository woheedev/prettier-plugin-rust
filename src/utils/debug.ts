...
const cwd = // @ts-expect-error
  typeof process === 'object' && typeof process?.cwd === 'function'
    ? /* @__PURE__ */ normPath(/* @__PURE__ */ process.cwd() ?? '')
    : '';
...
class StackItem extends StackLine {
  ...
  at(relIndex: number) {
    return this.i + relIndex >= this.stack.length || this.i + relIndex < 0
      ? undefined
      : this.stack[this.i + relIndex];
  }
  next() {
    return this.at(+1);
  }
...
function custom_prepareStackTrace(err, calls) {
  return (
    (Error_prepareStackTrace?.(err, calls) ?? calls.join('\n'))
      ?.replace(/file:\/\/\//g, '')
      .replace(/\\\\?/g, '/') ?? calls
  );
}
...
export function overrideDefaultError(silent = false) {
  ...
  if (!silent)
    console.log(color.grey(`[devtools] Replaced Error.prepareStackTrace at ${get_caller_cmd(1)}`));
}
...
function compose2Cols(left: string, right: string, len = 64, min = 1) {
  return (
    left +
    ' '.repeat(clamp(min, len, len - (color.unstyledLength(left) + color.unstyledLength(right)))) +
    right
  );
}
