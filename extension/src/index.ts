...
import plugin from '@woheedev/prettier-plugin-rust';
import {
  ExtensionContext,
  languages,
  Position,
...
console.log(
  `VSCode Extension: ${process.env.EXTENSION_NAME || 'jinxdash.prettier-rust'}@${process.env.EXTENSION_VERSION || 'dev'}`
);
    ...
    languages.registerDocumentFormattingEditProvider('rust', {
      async provideDocumentFormattingEdits(document) {
        ...
        console.log('', `# Formatting using prettier@${prettier.version}`, {
          ...config,
          plugins: config.plugins?.flatMap((p) =>
            typeof p === 'string' ? p : p.printers && Object.keys(p.printers)
          )
        });
        if (
          !config.plugins?.some(
            (plugin) =>
              typeof plugin === 'object' && plugin.languages?.some((lang) => lang.name === 'Rust')
          )
        ) {
